---
name: scrapegraph-x402
description: Call Scrapegraphai API API via x402 direct USDC payment. Available endpoints: GET /v1/searchscraper/{request_id}, GET /v1/markdownify/{request_id}, POST /v1/sitemap, POST /v1/markdownify, GET /v1/smartscraper/{request_id}, POST /v1/searchscraper, POST /v1/smartscraper, POST /v1/crawl, GET /v1/crawl/{task_id}, POST /v1/scrape, GET /v1/sitemap/{request_id}. Use for decentralized API payments. Requires PRIVATE_KEY with USDC on Base.
---

# Scrapegraphai API (x402 Payment)
...
When the user asks to use Scrapegraphai API with x402/crypto payment, use the code patterns below. The user's request is: $ARGUMENTS

**Base URL**: `https://x402.orth.sh/scrapegraph`
...
const response = await fetchWithPayment(
  "https://x402.orth.sh/scrapegraph/endpoint-path",  // Use appropriate endpoint
  {
...
1. Request is made to `https://x402.orth.sh/scrapegraph/...`
